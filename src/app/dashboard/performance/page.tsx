...
export default async function PerformancePage() {
  ...
  const avgMetrics = [
    { label: "Komunikasi", val: Math.round(metrics._avg.communication || 0), color: "bg-blue-500" },
    { label: "Teknikal", val: Math.round(metrics._avg.technical || 0), color: "bg-primary-container" },
    { label: "Kepercayaan Diri", val: Math.round(metrics._avg.confidence || 0), color: "bg-green-500" },
    { label: "Skor Akhir", val: Math.round(metrics._avg.overallScore || 0), color: "bg-accent-red" },
  ];
        ...
        <div className="bg-tertiary-container border-4 border-black p-8 shadow-brutal mt-8">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="bg-white border-2 border-black p-4 shadow-brutal-sm">
              ...
              <p className="text-sm font-medium">Kamu telah menyelesaikan total {interviews.length} sesi. Skor tertinggi kamu sejauh ini adalah {interviews.length > 0 ? Math.max(...interviews.map(i => i.overallScore)) : 0}.</p>
            </div>
            <div className="bg-white border-2 border-black p-4 shadow-brutal-sm">
